...
public class BallistaSingle extends BaseSingle {
    ...
    public BallistaSingle(double xInit,
                          double yInit,
    ...
    public void updateIntention(Terrain terrain) {
        ...
        switch (state) {
            ...
            case FIRE_AT_WILL:
                ...
                BaseUnit unitFiredAt = ((BallistaUnit)unit).getUnitFiredAgainst();
                if (unitFiredAt != null) {
                    reloadDelay -= 1;
                    if (reloadDelay == 0) {
                        // Pick a random target
                        shootingTarget = ((BallistaUnit) unit).pickNextTarget();

                        // Don't fire target if there is nothing to fire at.
                        if (shootingTarget == null) break;

                        // Shoot an arrow into the world
                        hasher.addObject(new Ballista(x, y, shootingTarget.getX(), shootingTarget.getY(),
