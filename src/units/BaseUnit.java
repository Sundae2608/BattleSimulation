...
public class BaseUnit implements Updatable {
    ...
    // Troops and width
    protected ArrayList<BaseSingle> troops;
    protected HashSet<BaseSingle> aliveTroopsSet;
    protected int width;
    protected int depth;
    protected HashSet<BaseSingle> borderedTroopSet;

    // Political attribute
    protected PoliticalFaction politicalFaction;
    ...
    // Unit state
    protected UnitState state = UnitState.STANDING;
    ...
    public void moveFormationKeptTo(double xGoal, double yGoal, double angleGoal) {
        ...
        currUnitPatience = unitPatience; // Reset patience.
        ...
        if (Math.abs(MathUtils.signedAngleDifference(anchorAngle, moveAngle)) > UniversalConstants.NON_UTURN_ANGLE) {
            ...
            anchorX += depth * spacing * downUnitX;
            anchorY += depth * spacing * downUnitY;
            // Flip the angle
            anchorAngle += Math.PI;
        }
    ...
    public void moveNoFormationTo(double xGoal, double yGoal, double angleGoal) {
        ...
        for (int i = 0; i < troops.size(); i++) {
            double xGoalSingle = goalX + (i % width) * spacing * sideUnitX + (i / width) * spacing * downUnitX;
            double yGoalSingle = goalY + (i % width) * spacing * sideUnitY + (i / width) * spacing * downUnitY;
    ...
    public void updateIntention() {
        ...
        switch (state) {
            ...
            case MOVING:
                ...
                anchorAngle = MovementUtils.rotate(anchorAngle, moveAngle, UniversalConstants.ANGLE_ROTATION_SPEED_ARMY);
                ...
                break;
            case STANDING:
                anchorAngle = MovementUtils.rotate(anchorAngle, goalAngle, UniversalConstants.ANGLE_ROTATION_SPEED_ARMY);
                isTurning = false;
        }
    ...
    public void updateGoalPositions() {
        ...
        for (int i = 0; i < troops.size(); i++) {
            double xGoalSingle = topX + (i % width) * spacing * sideUnitX + (i / width) * spacing * downUnitX;
            double yGoalSingle = topY + (i % width) * spacing * sideUnitY + (i / width) * spacing * downUnitY;
    ...
    private void updateBoundingBox() {
        ...
        double aliveHeight = aliveTroopsSet.size() * 1.0 / width * spacing;
        double fullToAliveRatio = 1.0 * troops.size() / aliveTroopsSet.size() ;

        double topLeftX = averageX - downUnitX * aliveHeight / 2 - sideUnitX * spacing * width / 2;
        double topLeftY = averageY - downUnitY * aliveHeight / 2 - sideUnitY * spacing * width / 2;

        double topRightX = averageX - downUnitX * aliveHeight / 2 + sideUnitX * spacing * width / 2;
        double topRightY = averageY - downUnitY * aliveHeight / 2 + sideUnitY * spacing * width / 2;

        double healthBotLeftX = averageX + downUnitX * aliveHeight / 2 - sideUnitX * spacing * width / 2;
        double healthBotLeftY = averageY + downUnitY * aliveHeight / 2 - sideUnitY * spacing * width / 2;

        double healthBotRightX = averageX + downUnitX * aliveHeight / 2 + sideUnitX * spacing * width / 2;
        double healthBotRightY = averageY + downUnitY * aliveHeight / 2 + sideUnitY * spacing *width / 2;
    ...
    public double[][] getGoalBoundingBox() {
        ...
        double topLeftX = goalX - sideUnitX * spacing * width / 2;
        double topLeftY = goalY - sideUnitY * spacing * width / 2;

        double topRightX = goalX + sideUnitX * spacing * width / 2;
        double topRightY = goalY + sideUnitY * spacing * width / 2;

        double botLeftX = goalX + downUnitX * aliveHeight - sideUnitX * spacing * width / 2;
        double botLeftY = goalY + downUnitY * aliveHeight - sideUnitY * spacing * width / 2;

        double botRightX = goalX + downUnitX * aliveHeight + sideUnitX * spacing * width / 2;
        double botRightY = goalY + downUnitY * aliveHeight + sideUnitY * spacing * width / 2;
