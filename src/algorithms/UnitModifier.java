...
public class UnitModifier {
    ...
    // Memoization of distance between units.
    private double[][] distanceMemo;

    public UnitModifier(ArrayList<BaseSingle> inputDeadContainer, ExperimentSettings inputSettings) {
        ...
        for (BaseObject obj : objects) {
            ...
            if (obj instanceof Arrow) {
                if (closestDistance < MathUtils.square(closestCandidate.getSize()) / 4) {
                    ...
                    closestCandidate.receiveDamage(ArcherConstants.ARROW_DAMAGE);
    ...
    private void modifyTroopsCollision() {
        ...
        if (experimentSettings.isBorderInwardCollision()) {
    ...
    private long modifySingleTroopPosition(BaseSingle troop) {
        ...
        for (BaseSingle candidate : candidates) {
            if (candidate.getPoliticalFaction() == troop.getPoliticalFaction() &&
                    ...
                    !(experimentSettings.isCavalryCollision() && (candidate instanceof CavalrySingle))) continue;
    ...
    private void modifyCombat() {
        ...
        // Shuffle to randomly settle tie between two units about to hit at the same time.
